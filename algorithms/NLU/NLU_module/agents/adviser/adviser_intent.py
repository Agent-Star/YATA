...
def run_intent_parsing(adviser, user_input: str, conversation_history: list = None, debug=False):
    """
    参数:
    ...
    needs_normalization = (
        not date_window or 
        not isinstance(date_window, dict) or
        (not date_window.get("from") and not date_window.get("to"))
    )
