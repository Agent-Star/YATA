...
"""
    ...
    markdown = await adviser.ask_text(
        itinerary_prompt, temperature=0.6, max_tokens=12000
    )
...
async def generate_itinerary_stream(
    adviser, result, rag_results, debug=False
    ...
    参数:
        adviser: Adviser 实例
        result: 包含 intent_parsed 等信息的结果字典
        rag_results: RAG 检索结果
        debug: 是否开启调试模式

    Yields:
        str: 每次生成的文本 chunk (Markdown 片段)
    """
    # 从意图里抓一些上下文（城市、日期等）
    intent = result.get("intent_parsed", {}) if isinstance(result, dict) else {}
    city = ""
...
"""
    ...
    try:
        ...
        if debug:
            logger.info("流式生成行程规划完成")

    except Exception as e:
        logger.error(f"流式生成行程失败: {e}")
        raise
