...
async def nlu_simple_api(request: NLURequest):
    ...
    try:
        ...
        itinerary_md = (
            result.get("itinerary_markdown")
            or result.get("detailed_itinerary", {}).get("itinerary_markdown")
        )

        recommend_md = (
            result.get("recommendations", {}).get("natural_summary")
            or result.get("recommendations", {}).get("summary")
        )

        general_text = (
            result.get("final_summary")
            or result.get("text_output")
        )
