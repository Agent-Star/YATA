...
⚠️ **重要**: `yata-tiny-frontend.service` 仅适用于用户级服务。如需使用系统级服务，请使用 `yata-tiny-frontend-system.service` 并根据实际情况修改用户名和路径。

```bash
# 1. 先编辑 yata-tiny-frontend-system.service，修改用户名和路径
# 对于 Ubuntu: User=ubuntu, 路径 /home/ubuntu/YATA/backend
# 对于 Amazon Linux: User=ec2-user, 路径 /home/ec2-user/YATA/backend

# 2. 复制到系统目录
sudo cp ~/YATA/backend/services/yata-tiny-frontend-system.service /etc/systemd/system/yata-tiny-frontend.service
```

详细的系统级服务配置说明请参考：[系统级服务配置说明.md](./系统级服务配置说明.md)
